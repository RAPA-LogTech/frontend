...
export default function RunbookDetailPage({
  ...
  const { data: runbook, isLoading, isFetched } = useQuery<RunbookDetail | null>({
  ...
  if (isFetched && !runbook) {
    return (
      <Box>
        <Button component={Link} href="/runbooks" startIcon={<BackIcon />} sx={{ textTransform: 'none', mb: 2 }}>
          Runbook List
        </Button>
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1.5, sm: 2, md: 3 } }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ sm: 'center' }} justifyContent="space-between" gap={1}>
        <Button component={Link} href="/runbooks" startIcon={<BackIcon />} sx={{ textTransform: 'none', alignSelf: 'flex-start' }}>
          Runbook List
        </Button>
        <Button variant="outlined" color="error" startIcon={<DeleteIcon />} onClick={() => setDeleteDialogOpen(true)} sx={{ textTransform: 'none' }}>
          Delete
        </Button>
      </Stack>
      ...
      {runbook && (
        <Card variant="outlined" sx={{ borderColor: 'divider' }}>
          <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Box>
              <Typography variant="h5" sx={{ fontWeight: 700 }}>{runbook.title}</Typography>
              <Stack direction="row" spacing={1} sx={{ mt: 1 }} flexWrap="wrap" useFlexGap>
                <Chip label={runbook.filename} size="small" variant="outlined" />
                <Chip label={`Updated: ${formatDateTime(runbook.updated_at)}`} size="small" variant="outlined" />
