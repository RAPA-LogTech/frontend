...
function StatRow({ label, value, color, sub }: { label: string; value: string; color: string; sub?: string }) {
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider' }}>
      ...
      <Typography variant="body1" sx={{ fontWeight: 800, color, fontFamily: 'monospace' }}>{value}</Typography>
    </Box>
  )
}

function MiniLineChart({ series, color, height = 60 }: { series: MetricSeries; color: string; height?: number }) {
    ...
    <Box sx={{ height }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={points} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
          ...
          <Tooltip
            contentStyle={{ backgroundColor: theme.palette.background.paper, border: `1px solid ${theme.palette.divider}`, borderRadius: 6, fontSize: 11 }}
...
export default function DatabaseTab({ metricSeries, rdsMetrics, envFilter, isLoading }: Props) {
  ...
  const hasRds = Boolean(rdsCpuSeries || rdsConnSeries || rdsFreeMemSeries || rdsReadLatSeries || rdsWriteLatSeries)
  ...
  // RDS 추이 차트 데이터
  const rdsTimes = buildTimeAxis([rdsCpuSeries, rdsConnSeries, rdsFreeMemSeries, rdsReadLatSeries, rdsWriteLatSeries])
      ...
      {hasPool && (
        <Paper variant="outlined" sx={{ p: 2, borderColor: 'divider', bgcolor: 'background.paper' }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'stretch' }}>
              ...
              <Box sx={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={poolChartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    <defs>
                      {[['active', '#60a5fa'], ['pending', '#fbbf24']].map(([k, c]) => (
                    ...
                    <YAxis tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} width={32} />
                    ...
                    {maxVal > 0 && <ReferenceLine y={maxVal} stroke="#4ade80" strokeDasharray="4 4" strokeWidth={1.5} label={{ value: 'max', position: 'insideTopRight', fontSize: 10, fill: '#4ade80' }} />}
                    <Area type="monotone" dataKey="active" name="Active" stroke="#60a5fa" fill="url(#pool-active)" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                    <Area type="monotone" dataKey="pending" name="Pending" stroke="#fbbf24" fill="url(#pool-pending)" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                  </AreaChart>
                </ResponsiveContainer>
              </Box>
            </Box>
            ...
            {/* 현재값 스탯 패널 */}
            <Box sx={{ flex: '0 0 220px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
              {/* 풀 사용률 게이지 */}
              <Box sx={{ mb: 1.5 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600 }}>Pool Utilization</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 800, color: utilPct >= 80 ? '#f87171' : utilPct >= 60 ? '#fbbf24' : '#4ade80' }}>
                    {utilPct.toFixed(1)}%
                  </Typography>
                </Box>
              ...
              <StatRow label="Pending Requests" value={pendingVal.toFixed(0)} color={pendingVal > 0 ? '#fbbf24' : 'text.secondary'} />
              <StatRow label="Max Pool Size" value={maxVal.toFixed(0)} color="#4ade80" />
              {useP95Val > 0 && <StatRow label="Use p95" value={`${useP95Val.toFixed(1)}ms`} color="#a78bfa" sub="connection acquire time" />}
              {waitP95Val > 0 && <StatRow label="Wait p95" value={`${waitP95Val.toFixed(1)}ms`} color="#fb923c" sub="queue wait time" />}
            </Box>
          </Box>
        </Paper>
      )}
      ...
      {hasRds && (
        <Paper variant="outlined" sx={{ p: 2, borderColor: 'divider', bgcolor: 'background.paper' }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'stretch' }}>
            ...
            {/* CPU + Connections 추이 */}
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block', mb: 1 }}>CPU & Connections</Typography>
              <Box sx={{ height: 180 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={rdsChartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid stroke={theme.palette.divider} strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="time" tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} minTickGap={30} />
                    <YAxis yAxisId="cpu" tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} width={32} tickFormatter={v => `${v}%`} />
                    <YAxis yAxisId="conn" orientation="right" tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} width={32} />
                    <Tooltip contentStyle={{ backgroundColor: theme.palette.background.paper, border: `1px solid ${theme.palette.divider}`, borderRadius: 8, fontSize: 12 }} />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Line yAxisId="cpu" type="monotone" dataKey="cpu" name="CPU %" stroke="#f87171" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                    <Line yAxisId="conn" type="monotone" dataKey="conn" name="Connections" stroke="#60a5fa" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                  </LineChart>
                </ResponsiveContainer>
              </Box>
            </Box>
            ...
            {/* Latency 추이 */}
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600, display: 'block', mb: 1 }}>Read / Write Latency</Typography>
              <Box sx={{ height: 180 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={rdsChartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid stroke={theme.palette.divider} strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="time" tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} minTickGap={30} />
                    <YAxis tick={{ fontSize: 10, fill: theme.palette.text.secondary }} tickLine={false} axisLine={false} width={40} tickFormatter={v => `${v}s`} />
                    <Tooltip contentStyle={{ backgroundColor: theme.palette.background.paper, border: `1px solid ${theme.palette.divider}`, borderRadius: 8, fontSize: 12 }} formatter={(v) => [`${Number(v).toFixed(4)}s`]} />
                    <Legend wrapperStyle={{ fontSize: 11 }} />
                    <Line type="monotone" dataKey="readLat" name="Read" stroke="#34d399" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                    <Line type="monotone" dataKey="writeLat" name="Write" stroke="#f87171" strokeWidth={2} dot={false} isAnimationActive={false} connectNulls />
                  </LineChart>
                </ResponsiveContainer>
              </Box>
            </Box>
            ...
            {/* RDS 현재값 스탯 */}
            <Box sx={{ flex: '0 0 200px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
              {rdsCpuSeries && <StatRow label="CPU Utilization" value={`${getSeriesLast(rdsCpuSeries).toFixed(1)}%`} color={getSeriesLast(rdsCpuSeries) >= 80 ? '#f87171' : '#4ade80'} />}
              {rdsConnSeries && <StatRow label="Connections" value={getSeriesLast(rdsConnSeries).toFixed(0)} color="#60a5fa" />}
              {rdsFreeMemSeries && <StatRow label="Freeable Memory" value={`${getSeriesLast(rdsFreeMemSeries).toFixed(0)} MB`} color="#fbbf24" />}
              {rdsReadLatSeries && <StatRow label="Read Latency" value={`${getSeriesLast(rdsReadLatSeries).toFixed(4)}s`} color="#34d399" />}
              {rdsWriteLatSeries && <StatRow label="Write Latency" value={`${getSeriesLast(rdsWriteLatSeries).toFixed(4)}s`} color="#f87171" />}
            </Box>
          </Box>
        </Paper>
      )}
