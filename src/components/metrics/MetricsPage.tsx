...
export default function MetricsPage({ currentTab = 'overview' }: { currentTab?: MetricTabKey }) {
  ...
  const { data: databaseMetricsData = EMPTY_METRICS, isLoading: isDatabaseMetricsLoading } =
    useQuery({
  ...
  const activeMetricSeries =
    currentTab === 'database'
      ? databaseMetricsData.length > 0
        ? databaseMetricsData
        : metricSeries
      : metricSeries

  const error4xxSeries = activeMetricSeries.filter(
    s => s.name.includes('4xx_ratio') && filterByEnv(s, envFilter)
  )
  const error5xxSeries = activeMetricSeries.filter(
    s => s.name.includes('5xx_ratio') && filterByEnv(s, envFilter)
  )
  ...
  const selectedTabIndex = Math.max(
    TAB_ITEMS.findIndex(item => item.key === currentTab),
    0
  )
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1.5, sm: 2, md: 3 } }}>
      ...
      >
        {TAB_ITEMS.map(item => (
          <Tab key={item.key} label={item.label} component={Link} href={item.href} disableRipple />
        ))}
      </Tabs>
